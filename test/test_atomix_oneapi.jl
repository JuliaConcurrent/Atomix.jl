...
@testset "AtomixoneAPIExt:test_complex_cas" begin
    A = oneAPI.zeros(ComplexF64, 3)
    oneapi() do
        GC.@preserve A begin
            ref = Atomix.IndexableRef(A, (1,))
...
@testset "AtomixoneAPIExt:test_complex_modify" begin
    A = oneAPI.fill(ComplexF64(1.0, 2.0), 3)
    oneapi() do
        GC.@preserve A begin
            ref = Atomix.IndexableRef(A, (1,))
...
@testset "AtomixoneAPIExt:test_complex_sugar" begin
    A = oneAPI.ones(ComplexF64, 3)
    oneapi() do
        GC.@preserve A begin
